  ...
  investmentXhr.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
      ...
      if (response == null) {
        ...
        document.getElementById("interest-account").innerText = response.investedAmount.toFixed(1);
...
function displayCryptoUpdateMobile(crypto, color, plus, direction) {
  ...
  <div class="w3-col s1"><img src="${crypto.image
	...
	<div class="w3-row">
							<div class="w3-col l6">
								<div class="w3-row">
									...
									<p class="small" style="margin: -8px 0px 8px; text-transform: uppercase">${crypto.symbol
    }</p>
									</div>
								</div>
							</div>
